                            ...
                            @foreach ($wishlists as $wishlist)
                                <tr>
                                    ...
                                    <form action="/wishlistDel" method="post">
                                        ...
                                        <button type="submit" role="button" class="btn btn-warning  text-white" style="margin-bottom:15px" name="delete" onclick="return confirm('are you sure?')">Delete</button>
                                    </form>
                                    <form action="/wishlistToCart" method="post">
                                        @csrf
                                        <input id=id type="hidden" name="id" value="{{ $wishlist->id }}">
                                        <input id=email type="hidden" name="email" value="{{ auth()->user()->email }}">
                                        <input id=product_id type="hidden" name="product_id" value="{{ $wishlist->product_id }}">
                                        ...
                                        <button type="submit" role="button" class="btn btn-warning  text-white" style="margin-bottom:15px" name="movetocart" onclick="return confirm('are you sure?')">move to cart</button>
                                    </form>
